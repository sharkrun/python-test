...
class Shell(threading.Thread):
    ...
    def run(self):
        while True:
            self.num += 1
            Log(3, "Shell run num:{}, current threading:{}".format(self.num, threading.active_count()))
            # self.timeout()
            if not self.timeout():
                break
    ...
    def is_login(self):
        Log(3, "is_login self.interval:{}, self.logined:{}".format(self.interval, self.logined))
        return self.logined
    ...
    def parse_user_prompt(self, message):
        Log(3, "parse_user_prompt:{}".format(message))
    ...
    def sent(self, message, encoding="cp1252"):
        Log(3, "sent message:{}".format(message))
        if message[0] == '[':
            return
        if not self.logined:
        ...
        Log(3, "Shell sent, sjon_msg:{}".format(json_msg))
